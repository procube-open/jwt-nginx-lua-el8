...
RUN yum -y update \
    ...
    && yum -y install unzip wget sudo git rpm-build gcc make openssl-devel pcre-devel
RUN mkdir -p /tmp/requires \
    ...
    && tar xvzf lua.tar.gz \
    && cd RPMS/x86_64/ \
    && rpm -ivh lua-5.1.4-15.el8.x86_64.rpm lua-devel-5.1.4-15.el8.x86_64.rpm luarocks-2.3.0-1.el8.x86_64.rpm
